...
def counting_sort():
    # Direct Access Table 개념이 들어간 내용이고 꽤 중요한 내용
    # 현업에서 직접 자료구조를 다루던, 해시나 DB등에서 간접적으로 다루게 됨
    # 인덱스 테이블, 직접 접근 개념 들어감
    start_time = time.time()
    # 0~1000 사이 값을 가지는 랜덤 정수가 1000개 든 배열 생성 
    arr = [random.randint(0,1000) for _ in range(1000)] 
    N = len(arr)
...
def exaustive_search():
    ...
    현업에선 단순 시뮬레이션, 경로탐색, 게임 로직(좀 많이 멍청하고 수동적인AI) 등에서 
...
def greedy_algorithm():
    ...
    전역 최적해(Global Optimum)를 절대 보장하지 않음.
    ...
    이게 최적해인지 아닌지 따지기 귀찮을때
    "그리디 알고리즘" 이라고 둘러대면서 표현해서 반영함. 
    ...
    num = 112233 # 
    c = [0] * 12 #6자리 수 세서 저장할 count list
