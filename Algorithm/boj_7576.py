...
# 모두 익지 못하는 상황을 찾는다. 전체 탐색하면서 델타 돌려서 같힌 케이스가 생기면 break 하고 종료
# 만약 가능하다면, 다중 큐 만들어서 거리 잰다.
...
while head < len(queue):
    ...
    for k in range(4):
        ni = i + delta[k][0]
        nj = j + delta[k][1]
        if 0 <= ni < N and 0 <= nj < M and visits[ni][nj] == -1:
            visits[ni][nj] = visits[i][j] + 1
            queue.append((ni, nj))
max_cnt = 0
flag = True
for i in range(N):
    ...
    for j in range(M):
        if visits[i][j] == -1:
            ...
            max_cnt = max(max_cnt, visits[i][j])
