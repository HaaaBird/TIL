...
걍 어제 문제랑 동일한거 같은데
리스트로 배열 받은 다음에 백트래킹 써서 1~6까지 굴려보고 가지치기
다만 값이 100을 넘어가면 안됨 딱 100을 만들어야 함.
...
def backt(np, cnt):
    ...
    if np == 100:
        ...
        # 100을 초과했음 다시 돌아가기
        return
    # 가지치기 조건. 이미 갱신된 min_cnt 보다 주사위를 더 많이 던졌다면
    if min_cnt <= cnt:
        return
    for i in range(1, 7):
        np += i
...
ladder = {k:v for k,v in [list(map(int, input().split())) for _ in range(N)]}
snake = {k:v for k,v in [list(map(int, input().split())) for _ in range(M)]}
