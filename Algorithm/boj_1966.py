...
"""
문제
여러분도 알다시피 여러분의 프린터 기기는 여러분이 인쇄하고자
하는 문서를 인쇄 명령을 받은
‘순서대로’,
즉 먼저 요청된 것을 먼저 인쇄한다.
...
for case in range(1, T + 1):
    ...
    # [0, 0], 시작인덱스 벨류
    shift_list = [[i, arr[i]] for i in range(N)]
    cnt = 0

    while len(shift_list) != 0:
        for i in range(len(shift_list)):
            if shift_list[0][1] < shift_list[i][1]:
                temp = shift_list.pop(0)
                shift_list.append(temp)
                break
            elif i == len(shift_list)-1:
                cnt += 1
                now_pop = shift_list.pop(0)
                if now_pop[0] == M:
                    print(cnt)
