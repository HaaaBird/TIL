...
### 수열 bj_2559
- 작성일자: 2025-07-23
- [2669](https://www.acmicpc.net/problem/2559)
- [내 코드](bj_2559.py)
- 문제설명
    - 정수가 나열된 리스트가 제공되고, 합칠 구간을 준다.
    - 예를 들어, 길이가 10개인 리스트고, 합칠 구간이 2라면
    - list[0] + list[1], list[1] + list[2] ... 이런식으로
    - 이 값들 중 최대값을 출력하는 문제이다.
- 문제풀이
    - 설명에 힌트를 넣었는데, 누적합을 구해서 풀어야 한다.
    - 단순히 구간별 덧샘 해서 풀면 시간 초과로 틀린다. 아래는 틀린 코드 예
    ```python
    list_len, distance = map(int, input().split())
    temp_list = list(map(int, input().split()))

    result = 0
    for i in range(list_len - distance + 1):
        if result < sum(temp_list[i:i+distance]): result = sum(temp_list[i:i+distance])

    print(result)

    ```
    - 왜 시간초과냐? 문제에서 주어진 시간 제한이 1초인데, 입력 제약조건에 주어진 조건이
        - 리스트 길이는 2~ 100,000
        - 리스트 안에 들어가는 합칠 날짜 구간의 수가 1~100,000 사이의 정수
        - 계산 횟수를 따져보면
            - 계산횟수 = 리스트 길이 - 구간길이 + 1
            - 만약 리스트 길이가 10만, 구간길이가 5만 이면
            - 100,000 - 50,000 + 1 = 50,001 번의 구간에 대해 SUM 연산을 해야하고
            - 50,000 X 50,000 =2.5억번 연산임으로, 제한시간인 1초 안에 수행하는것은 어려움.
    - 그럼 시간 초과를 피하는 방법은?
        - 구간합을 이용해 문제 풀면 됨.
        - 리스트 받아온 후, 구간합 리스트를 만들고 인덱스를 맞춘 다음
        - 각각의 구간합 특성을 이용해 두 숫자 뺄셈을 통해 구하면 훨씬 더 가벼운 연산으로 수행 가능.
### 빙고판 bj_2578
- 작성일자: 2025-07-23
- [2669](https://www.acmicpc.net/problem/2578)
- [내 코드](bj_2559.py)
- 문제설명
    - 5x5 빙고를 판을 주고, 마찬가지로 빙고 숫자를 불러주는데, 3빙고가 나오면 승리다.
    - 진행자는 숫자를 1개씩 외치는걸로 생각한다. 그럴때 몇 번째로 외쳤을때 3빙고가 되는가를 출력하면 됨
- 내 풀이
    - 가로 확인, 세로 확인, 크로스 확인 로직을 만든다.
    - 진행자가 하나 부를때마다 3개 확인 로직을 돌리고, 몇 빙고인지 확인한다.
    - 3빙고가 넘으면 break 걸고 중단한다
- 풀면서 느낀 것
    - 배열의 좌표는 x,y로 우리는 통상적으로 생각하지만, 2차원 배열에서는 array[y][x]이다. 이거 자꾸 햇갈리는데, 많이 풀어봐야 적응할듯