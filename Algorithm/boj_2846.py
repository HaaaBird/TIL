...
"""
문제
상근이는 자전거를 타고 등교한다. 자전거 길은
오르막길, 내리막길, 평지로
이루어져 있다. 상근이는 개강 첫 날 자전거를 타고 가면서 일정 거리마다 높이를 측정했다.
...
for i in range(1, N):
    if arr[i] <= arr[i-1]:  # 오름차순 끊김
        max_high = max(max_high, arr[i-1] - start)
        start = arr[i]
    if i == N-1:  # 마지막 원소에서 구간 종료
        max_high = max(max_high, arr[i] - start)
